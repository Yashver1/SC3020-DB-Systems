...
bool bplustree::search (float key, unsigned &offset){
    ...
    while(!node.isLeaf){
        unsigned i = 0; 
        //if the search key is greater than or equal to the current key, moves to the next key
        while (i<node.numKeys && key >= node.keys[i]) i++;
        block.loadAt(node.children[i]);
    }
...
void bplustree::splitNode(bplustreeNode &node, unsigned offset){
    ...
    newnode.children[newnode.numKeys]= node.children[node.numKeys]; //update the child's pointer
